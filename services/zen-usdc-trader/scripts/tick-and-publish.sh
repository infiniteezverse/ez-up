...
# 2. If history.json changed (per git), commit & push
cd "$REPO_ROOT"
if ! git diff --quiet -- "$HISTORY_FILE"; then
  echo "[$(date -u +%FT%TZ)] history.json changed — pushing"
  git add "$HISTORY_FILE"
  git commit -m "chore: update bot history snapshot ($(date -u +%FT%TZ))"
  git push origin main
  echo "[$(date -u +%FT%TZ)] published"
else
  echo "[$(date -u +%FT%TZ)] history.json unchanged — skipping push"
fi
